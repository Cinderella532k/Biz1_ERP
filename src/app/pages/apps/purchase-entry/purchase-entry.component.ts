...
export class PurchaseEntryComponent implements OnInit {
  ...
  addcustomer() {
    this.Auth.addCustomer(this.customerdetails).subscribe(
      data => {
        console.log(data)
        this.notification.success(
          'Customer Added!',
    ...
    if (this.validation()) {
      ...
      if (
        this.order.Items.some(
          x =>
            x.BarcodeId == this.temporaryItem['barcodeId'] &&
            x.Price == this.temporaryItem['price'],
        )
      ) {
      ...
      if (
        ...
        this.order.Items.filter(
          x =>
            x.BarcodeId == this.temporaryItem['barcodeId'] &&
            x.Price == this.temporaryItem['price'],
        )[0].ReceivedQuantity += this.temporaryItem.rQuantity
    ...
    if (this.VendorsValidation()) {
      ...
      if (!this.order.Id) {
        this.Auth.saveorderdb(this.order).subscribe(data => {
          this.Auth.directreceivedpurchase(
            this.orderId,
            this.order.OrderedById,
            this.order,
          ).subscribe(data1 => {
            console.log(data1)
            this.vendorsubmitted = false
        ...
        this.Auth.updatereceivedItem(this.orderId, this.order.OrderedById, this.order).subscribe(
          data => {
            this.order = null
  ...
  updateReceivedOrder() {
    this.Auth.updatereceivedPurchase(this.orderId, this.order.OrderedById, this.order).subscribe(
      data => {
        this.order = new OrderModule(1)
